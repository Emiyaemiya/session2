...
%token IF ELSE FOR WHILE CONTINUE BREAK RETURN CONST
...
%nterm <std::vector<FE::AST::ExprNode*>*> PARAM_ARRAY_DIMS
%nterm <std::vector<FE::AST::ExprNode*>*> PARAM_ARRAY_DIMS_TAIL
...
IF_STMT:
    ...
    ;//使用%prec THEN来实现对移入的控制，then优先级小于else，当stmt后为else时，优先选择高优先级，即else移入。
...
// 参数多维数组维度：首维可为空 [] 表示未定长(-1)，后续维度为表达式
PARAM_ARRAY_DIMS:
    LBRACKET RBRACKET PARAM_ARRAY_DIMS_TAIL {
        auto v = new std::vector<ExprNode*>();
        v->emplace_back(new LiteralExpr(-1, @1.begin.line, @1.begin.column));
        if ($3 && !$3->empty()) v->insert(v->end(), $3->begin(), $3->end());
        $$ = v;
    }
    | LBRACKET NOCOMMA_EXPR RBRACKET PARAM_ARRAY_DIMS_TAIL {
        auto v = new std::vector<ExprNode*>();
        v->push_back($2);
        if ($4 && !$4->empty()) v->insert(v->end(), $4->begin(), $4->end());
        $$ = v;
    }
    ;
...
PARAM_ARRAY_DIMS_TAIL:
    ...
    | PARAM_ARRAY_DIMS_TAIL LBRACKET NOCOMMA_EXPR RBRACKET {
        $$ = $1;
        $$->push_back($3);
    }
    ;
